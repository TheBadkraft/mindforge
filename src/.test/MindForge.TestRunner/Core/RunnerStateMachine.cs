...
public abstract class RunnerStateMachine<TState> where TState : struct, IConvertible, IComparable, IFormattable
{
    ...
    /// </summary>
    protected RunnerStateHandler<TState> StateHandler;

    /// <summary>
    /// Initializes a new instance of the <see cref="RunnerStateMachine{TState}"/> class with the specified initial state and state handler.
    /// </summary>
    ...
    protected RunnerStateMachine(TState initialState, RunnerStateHandler<TState> stateHandler)
    {
